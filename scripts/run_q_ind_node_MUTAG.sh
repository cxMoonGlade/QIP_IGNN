...
#
# Usage:
#   bash scripts/run_q_ind_node_MUTAG.sh
#   GPUS="0 1 2 3" SESSION=q_ind_node_MUTAG \
#     CONDA_ENV=qenv2 bash scripts/run_q_ind_node_MUTAG.sh
# ======================================================================
set -euo pipefail
cd "$(dirname "$0")/.."
PROJECT_DIR=$(pwd)
...
TMP_TASK_FILE=$(mktemp /tmp/qignn_mutag_q_ind_node_XXXX.txt)
...
export GPUS="${GPUS:-0}"
export SESSION="${SESSION:-q_ind_node_MUTAG}"
